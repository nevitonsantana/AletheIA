...
- Discoverability and first-use distribution: the public APM package is available as immutable `v1.0.1-apm` for Claude and Codex; the first disposable install/scaffold smoke test passed using the documented direct-script fallback because APM `v0.26.0` did not expose dependency scripts through `apm run`.
- GitHub Pages: the current documentation source was manually built, validated and deployed successfully from `main`; ongoing adoption remains an observation activity, not a new evolution initiative.
...
1. Keep `Active: none` after the S86 `insufficient_evidence` decision; do not treat the source pack, schemas or prompts as implementation authority.
...
- **Review trigger:** reconciliation after public APM first-use validation and GitHub Pages publication
