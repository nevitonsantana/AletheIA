...
| P10/P12 — Work Observatory | `partially-delivered` | Four derived `work_record` examples over S7, S12, S21 and S25 evidence; logical sublayer inside Observability & Intelligence | Accumulate comparable records without activating work units, value scoring or comparative dashboards. |
...
- **Evidence:** merge `5835670` adds the first metadata-only derived record and guardrail tests for S7. Later S12, S21 and S25 dogfood records add source-backed work observations for agent-role reconciliation, independent validation hardening and human-expertise learning. All keep unsupported duration, resource, acceptance, rework, work-unit, quality and value claims explicitly `unavailable` or `unknown`.
...
- **Status:** `partially-delivered`; four real derived records exist, while calibrated and comparative capabilities remain deferred.
