...
| AletheIA Portable Governed Work Evolution Pack v0.1 | P25/S83 intake-only source narrowed to optional portability metadata around the existing Restart Package; no new canonical artifact or implementation authority | `d2e91b2429245472418b2c0a84b0277da34670505e222fe762fb763d91886f21` |
...
|---|---|---|---|
| P0 — Coding Safety Workflow | `delivered` | Coding specialization of the existing Work Slice plan | Delivered by S9 through the minimum coding safety profile without runtime, command, auto-merge or skill promotion. |
...
| P25 — Portable Continuity / Restart Package Portability v0.1 | `in-review` for intake only | Existing Restart Package, handoff and finalization boundaries | S83 registers the source and rejects a second canonical continuity artifact; only S84 compatibility review may follow the intake gate. |
                                                      ...
                                                      -> S49 Next-source backlog gate
                                                          -> S50 Learning Distillation & Context Hygiene Addendum v0.7 intake review
                                                              -> S51 Restart Package learning and context hygiene compatibility review
                                                                  -> S52 AletheIA to Adaptive Skills evolution bridge compatibility review
                                                                      -> S53 Minimum sufficient solution guardrail compatibility review
                                                                          -> S54 Observatory interface refinement review
```

The post-S54 documentation path through S82 is delivered. The new source-triggered continuity path is separate and conditional:

```text
S82
  -> S83 Portable Continuity Reference Intake
      -> intake gate: S84 only
          -> S84 Restart Package portability compatibility review
              -> compatibility gate: absorb/stop or authorize S85
                  -> S85 Codex-to-Codex restart pilot
                      -> S86 same-runtime value decision
...
- **Repository / layer:** AletheIA domain governance usage evidence
- **Covers:** real/source-backed S28/S29 application to Dependabot dependency PRs #343, #344 and #345 as external automated trust-boundary inputs before their later dependency-maintenance merges.
...
- **Evidence:** merge `5835670` adds the first metadata-only derived record and guardrail tests for S7. Later S12, S21, S23 and S25 dogfood records add source-backed work observations for agent-role reconciliation, independent validation hardening, loop readiness and human-expertise learning. All keep unsupported duration, resource, acceptance, rework, work-unit, quality and value claims explicitly `unavailable` or `unknown`.
...
- **Dependencies:** S13
- **Status:** `delivered` by AletheIA PR #266 and Adaptive Skills PR #68
...
- **Evidence:** AletheIA merge `ec9e9af` adds the state contract, root index, Restart Package compatibility fields, post-resume check, context-surface entries and tests; Adaptive Skills merge `baf4a4a` adds its local state, context entry, validator and required CI gate.
...
- **Status:** `delivered` through AletheIA PR #289.
...
- **Evidence:** AletheIA PR #289 / `0800cd0`; human expertise and evidence-based learning contract, starter-pack templates, synthetic governed slice and contract tests.
...
- **Status:** `delivered` through ADR-016 / AletheIA PR #295
...
- **Status:** `delivered` as a docs-tooling spike; production publishing remains gated.
...
- **Status:** `delivered`; publication remains gated.
...
- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Decision map for which repository areas belong in the official Blume documentation site, which remain GitHub source links and which stay out of publication scope.
- **Dependencies:** S55 Blume docs shell spike and S56 Blume validation/source-coverage triage.
- **Status:** `delivered`; publishing remains gated.
- **Non-goals:** publishing GitHub Pages, changing Blume config, fixing all links, broad documentation rewrite, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** record publication tiers and link-handling rules that unblock future small link-repair batches without turning the repository into a full docs mirror.
- **Evidence:** `docs/reports/blume-publication-source-map-s57.md`.

### S58 — Blume internal docs route repair

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Narrow repair of Blume validation failures caused by links that should resolve inside the existing `docs/` source.
- **Dependencies:** S55 Blume docs shell spike, S56 validation/source-coverage triage and S57 publication-source map.
- **Status:** `delivered`; publishing remains gated.
- **Non-goals:** publishing GitHub Pages, changing Blume config, expanding source coverage, fixing all links, broad documentation rewrite, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** reduce internal `docs/` route validation failures while leaving non-`docs/` source coverage decisions to later S57-category slices.
- **Evidence:** `docs/reports/blume-internal-route-repair-s58.md`, `pnpm run docs:validate`.

### S59 — Blume root/meta and policy source-link handling

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Narrow handling of Blume validation failures caused by repository-root, repository-meta and policy references that are canonical source files outside the `docs/` Blume source tree.
- **Dependencies:** S55 Blume docs shell spike, S56 validation/source-coverage triage, S57 publication-source map and S58 internal docs route repair.
- **Status:** `delivered`; publishing remains gated.
- **Non-goals:** publishing GitHub Pages, changing Blume config, expanding source coverage, fixing examples/starter-pack/packs/assets, broad documentation rewrite, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
...
- **Evidence:** `docs/reports/blume-starter-pack-source-links-s61.md`, `pnpm run docs:validate`; PR #389.

### S62 — Blume pack source-link handling

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Narrow handling of Blume validation failures caused by `docs/` links to repository-level `packs/operating-overlay/` delivery pack material.
- **Dependencies:** S55 Blume docs shell spike, S56 validation/source-coverage triage, S57 publication-source map, S58 internal docs route repair, S59 root/meta/policy source-link handling, S60 Visual Operations example source-link handling and S61 starter-pack source-link handling.
- **Status:** `delivered`; publishing remains gated.
- **Non-goals:** publishing GitHub Pages, changing Blume config, moving packs into `docs/`, fixing non-Visual-Operations examples/assets, broad documentation rewrite, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
...
- **Status:** `delivered`; publishing remains gated pending asset-check review.
...
- **Evidence:** `docs/reports/blume-example-source-links-s63.md`, `pnpm run docs:validate`; PR #393.

### S64 — Blume asset-check warning readiness review

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Narrow review of the remaining `BLUME_ASSETS_UNCHECKED` warning after S63 cleared broken-link validation errors.
- **Dependencies:** S55 Blume docs shell spike, S56 validation/source-coverage triage, S57 publication-source map, S58 internal docs route repair, S59 root/meta/policy source-link handling, S60 Visual Operations example source-link handling, S61 starter-pack source-link handling, S62 pack source-link handling and S63 non-Visual-Operations example source-link handling.
- **Status:** `delivered`; official publishing remains gated pending a deliberate asset-publication strategy or documented acceptance of the warning.
- **Non-goals:** publishing GitHub Pages, changing Blume config, copying or relocating assets, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
...
- **Status:** `delivered`; official publishing remains gated by a separate publication-readiness decision.
...
- **Evidence:** `docs/reports/blume-asset-warning-decision-s65.md`, `pnpm run docs:validate`, `pnpm run docs:build`; PR #397.

### S66 — Blume publication-readiness decision

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Decision on hosting target, expected base URL, base path and publication gates for a future Blume documentation deployment.
- **Dependencies:** S55 Blume docs shell spike, S56 validation/source-coverage triage, S57 publication-source map, S58 internal docs route repair, S59 root/meta/policy source-link handling, S60 Visual Operations example source-link handling, S61 starter-pack source-link handling, S62 pack source-link handling, S63 non-Visual-Operations example source-link handling, S64 asset-check warning readiness review and S65 asset-check warning acceptance decision.
- **Status:** `delivered`; no deployment or workflow changes are included.
- **Non-goals:** publishing GitHub Pages, changing Blume config, changing repository Pages settings, creating deployment workflow, automatic documentation generation, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** document the intended future hosting target and the required gates for a separate publishing implementation slice.
- **Evidence:** `docs/reports/blume-publication-readiness-decision-s66.md`, `pnpm run docs:validate`, `pnpm run docs:build`; PR #399.
...
- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Evidence from the first explicit manual GitHub Pages deployment and S67 smoke test.
- **Dependencies:** S67 Blume GitHub Pages readiness implementation and explicit human approval to run the manual workflow.
...
- **Acceptance:** successful manual workflow run and smoke-test evidence for home page, representative route, assets, `llms.txt`, `robots.txt`, sitemap and search index.
- **Evidence:** `docs/reports/blume-pages-deploy-smoke-test-s68.md`, GitHub Actions run `29390178825`, `https://nevitonsantana.github.io/AletheIA/`; PR #403.

### S69 — Blume docs output audit and postprocess guard

- **Repository / layer:** AletheIA documentation tooling
- **Covers:** Published Blume artifact hygiene for duplicate first headings, internal Markdown links and raw Markdown files.
- **Dependencies:** S68 public Pages smoke test and review of the generated GitHub Pages surface.
- **Status:** `delivered`; automatic publishing on merge remains disabled.
- **Non-goals:** source Markdown rewrite, automatic GitHub Pages publishing on merge, custom domain, generated documentation pipeline, dashboards, collectors, scoring, ranking, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `pnpm run docs:build` runs postprocess/audit and the resulting `apps/docs/dist` has zero raw Markdown artifacts, zero internal `.md`/`.mdx` links and zero duplicate first headings.
- **Evidence:** `docs/reports/blume-output-audit-s69.md`, `scripts/postprocess-blume-docs.mjs`, `scripts/audit-blume-docs-output.mjs`; PR #404.
...
- **Status:** `delivered`; language normalization remains a separate follow-up decision.
...
- **Evidence:** `docs/reports/docs-language-and-adr-navigation-audit-s71.md`, `docs/meta.ts`, `docs/adr/meta.ts`; PR #407.

### S72 — Public docs English language policy

- **Repository / layer:** AletheIA documentation content governance
- **Covers:** Official language decision for published public documentation before any translation/normalization work.
- **Dependencies:** S71 docs language and ADR navigation audit plus explicit user decision that English is the official public documentation language.
- **Status:** `delivered`; translation work remains a separate follow-up slice.
- **Non-goals:** bulk translation, automatic language classification, rewriting historical evidence, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** public-doc language policy records English as the official language and defines boundaries for preserving historical evidence.
- **Evidence:** `docs/reference/public-doc-language-policy.md`; PR #409.

### S73 — Core concept English normalization

- **Repository / layer:** AletheIA public documentation content
- **Covers:** First small English-normalization slice for public concept pages identified by S71.
- **Dependencies:** S72 public docs English language policy.
- **Status:** `delivered`; further mixed-language pages remain follow-up work.
- **Non-goals:** bulk translation, rewriting historical evidence, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `docs/concepts/governance.md` and `docs/concepts/quality.md` are normalized to English while preserving meaning and existing public-doc scope.
- **Evidence:** `docs/concepts/governance.md`, `docs/concepts/quality.md`; PR #411.

### S74 — Launch Kit English normalization

- **Repository / layer:** AletheIA public documentation content
- **Covers:** English-only normalization of the public Launch Kit after S72 established English as the canonical docs language.
- **Dependencies:** S72 public docs English language policy and S73 first concept normalization.
- **Status:** `delivered`; further mixed-language pages remain follow-up work.
- **Non-goals:** bulk translation, rewriting historical evidence, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `docs/reference/launch-kit.md` is English-only and no longer carries duplicate Portuguese announcement copy in the canonical public docs surface.
- **Evidence:** `docs/reference/launch-kit.md`; PR #413.

### S75 — Architecture flow language normalization

- **Repository / layer:** AletheIA public documentation content
- **Covers:** Residual Portuguese flow label in the public architecture concept page.
- **Dependencies:** S72 public docs English language policy and S74 Launch Kit normalization.
- **Status:** `delivered`; further mixed-language pages remain follow-up work.
- **Non-goals:** bulk translation, rewriting historical evidence, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `docs/concepts/architecture.md` uses English for the core contract flow label.
- **Evidence:** `docs/concepts/architecture.md`; PR #415.

### S76 — ADR-007 language normalization

- **Repository / layer:** AletheIA public ADR documentation
- **Covers:** Residual Portuguese terms and one typo in ADR-007's APM packaging decision narrative.
- **Dependencies:** S72 public docs English language policy and S75 architecture flow language normalization.
- **Status:** `delivered`; broader ADR language review remains follow-up work.
- **Non-goals:** rewriting ADR decisions, changing APM strategy, bulk translation, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `docs/adr/ADR-007-apm-packaging-strategy.md` uses English for the decision narrative terms touched by this slice and fixes the visible `nativ ely` typo.
- **Evidence:** `docs/adr/ADR-007-apm-packaging-strategy.md`; PR #417.

### S77 — KGL prep language normalization

- **Repository / layer:** AletheIA public roadmap documentation
- **Covers:** Residual Portuguese phase label in the Knowledge Governance Layer technical implementation prep roadmap.
- **Dependencies:** S72 public docs English language policy and S76 ADR-007 language normalization.
- **Status:** `delivered`; visual/Kanban PT-BR labels remain a separate follow-up because they may be communication labels rather than accidental language drift.
- **Non-goals:** changing KGL implementation scope, rewriting roadmap history, translating visual labels, bulk translation, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** `docs/roadmaps/knowledge-governance-implementation-prep.md` uses English for the Phase 5 implementation-prep label.
- **Evidence:** `docs/roadmaps/knowledge-governance-implementation-prep.md`; PR #419.

### S78 — Public language residual classification

- **Repository / layer:** AletheIA public documentation governance
- **Covers:** Classification of remaining Portuguese-language surfaces after S77.
- **Dependencies:** S72 public docs English language policy and S77 KGL prep language normalization.
- **Status:** `delivered`; no further automatic translation is admissible from this scan.
- **Non-goals:** translating historical learning records, rewriting pilot/local guides, changing visual/Kanban labels, bulk translation, automatic language classification, publishing automation, dashboards, collectors, scanners, policy engines, S18 metrics, Runtime 2.0 behavior or Adaptive Skills mutation.
- **Acceptance:** a review report identifies remaining language categories and records the decision boundary for future editorial action.
- **Evidence:** `docs/reports/public-language-residual-classification-s78.md`; PR #421.

### S79 — Open Dependabot trust-boundary review
...
- **Repository / layer:** AletheIA reference intake and backlog planning.
- **Covers:** P25 portable continuity, runtime-boundary metadata, Restart Package reuse and the source pack's broader loop/graph/topology proposals as external planning data.
- **Dependencies:** S2 reference intake protocol, S82 closure, current Restart Package/handoff/finalization contracts and ADR-016.
- **Status:** `in-review`; no implementation authority is introduced.
- **Non-goals:** a Portable Governed Work State, canonical schema, composer, runtime adapter, template change, pilot execution, loop/graph/topology expansion, Runtime 2.0, memory writeback, automatic routing, observatory changes or Adaptive Skills mutation.
- **Acceptance:** register checksum and provenance; map requirements to existing contracts; record adopted, absorbed, deferred and rejected elements; preserve Restart Package as the only continuity artifact; authorize at most S84 compatibility review.
- **Evidence:** `examples/reference-intake/s83-portable-continuity-intake.yaml`; local S83 validation.
...
- **Status:** `planned`; not active until S83 is accepted.
...
### S87 — Optional Restart Package portability block

- **Repository / layer:** AletheIA handoff/finalization templates.
- **Dependencies:** S86 `accept_optional_profile`.
- **Status:** `deferred`.
- **Non-goals:** changing the minimum handoff schema, creating a new artifact, composer or runtime adapter.
- **Acceptance:** add one optional block only where a real runtime or trust boundary exists; absent values remain `unavailable`.

### S88 — Bidirectional cross-runtime portability pilot

- **Repository / layer:** AletheIA project-specific continuity evidence.
- **Dependencies:** accepted S87 optional block.
- **Status:** `deferred`.
- **Routes:** Codex to Claude Code and Claude Code to Codex, read-only at the post-resume checkpoint.
- **Non-goals:** Adaptive Skills changes, instruction enforcement claims, automatic runtime configuration or execution automation.
- **Acceptance:** preserve constraints, validation, sensitivity and unsupported semantics in both routes with complete evidence and independent review.

### S89 — Restart Package portability adoption decision

- **Repository / layer:** AletheIA continuity governance.
- **Dependencies:** S88 cross-runtime evidence.
- **Status:** `deferred`.
...
| P0-R2 Advisor/executor/verifier/reconciler separation | P0 PRD | `delivered` | S9, S12 | S9 covers plan/review boundaries; S12 reconciles implementer/reviewer role boundaries. |
...
| P7-R1 Portable professional roles | P7 PRD | `delivered` | S12 | Existing role catalog remains canonical; professional labels are projections. |
| P7-R2 Role authority, evidence, stop and output contracts | P7 prompt assessment | `delivered` | S12 | S12 adds the missing fields through a role plan record. |
| P7-R3 Skill-provider independence | P7 policies | `delivered baseline` | S10, S12 | Runtime/provider mapping remains optional and does not redefine portable roles. |
...
| P11-R6 Documentation-health projection | P11 integration map | `delivered` for public language residual classification; automatic publishing remains disabled | S55, S56, S57, S58, S59, S60, S61, S62, S63, S64, S65, S66, S67, S68, S69, S70, S71, S72, S73, S74, S75, S76, S77, S78, S80 | S55 proved an isolated static documentation shell can build from existing Markdown; S56 classified validation/source-coverage gaps; S57 mapped publication sources; S58 repaired internal docs routes; S59 handled repository-root, `_meta` and policy references; S60 handled Visual Operations examples; S61 handled starter-pack artifacts; S62 handled operating-overlay pack material; S63 cleared remaining example broken links; S64 documented the asset-check warning; S65 accepted the warning for the spike; S66 selected the future host; S67 added manual-only Pages readiness; S68 records explicit manual deployment and smoke-test evidence; S69 guards the generated artifact against duplicate headings, internal Markdown links and raw Markdown files; S70 records the post-guard manual publication smoke test; S71 adds explicit ADR navigation metadata and a language-coherence audit; S72 records English as the public-doc language; S73 starts small public concept normalization; S74 normalizes the public Launch Kit; S75 removes a residual Portuguese architecture flow label; S76 normalizes residual ADR-007 Portuguese terms and a visible typo; S77 normalizes the KGL prep phase label; S78 classifies remaining language residuals and blocks further automatic translation before any bulk translation, automatic publishing, mutation or documentation-health projection. S79 separately records a new real Dependabot trust-boundary review without changing documentation-health projection. S80 resolves the remaining S78 language residual categories through explicit editorial decisions rather than automatic translation. S81 records the post-S80 backlog gate and keeps future movement source-triggered. S82 records the manual post-S81 Pages publication and keeps automatic publishing blocked. |
...
| P25-R2 Runtime, freshness and sensitivity portability metadata | Portable Continuity Pack PRD/SDD | `in-review` | S83/S84 | S83 registers a residual-gap hypothesis only; S84 must absorb it into existing fields or justify one optional block. |
...
- **Status:** `delivered`; start with the smallest Domain Expertise Brief and Evidence-Based Learning Record.
...
- **Evidence:** AletheIA PR #289 / `0800cd0`; human expertise and evidence-based learning contract, starter-pack templates and synthetic example.
- **Deferred:** capability fitness dashboards, rankings and learning automation.

### P14 — Governed Loop Engineering Addendum

- **Status:** `delivered`; reconciled with existing execution-pattern and loop contracts.
...
- **Evidence:** AletheIA PR #283 / `0b0531a`; S23 Loop Readiness Gate contract, review checklist, template, synthetic fixture and guardrail tests.
...
- **Status:** `delivered`; prioritize its guardrails before broader automation.
...
- **Evidence:** AletheIA PR #281 / `fb9f900`; S21 contract, blind-review checklist, starter-pack template, synthetic review case and contract tests.
...
- **Status:** `delivered` for boundary review; ADR-016 keeps it as a strategic north-star and boundary-review source.
...
### P25 — Portable Continuity / Restart Package Portability v0.1

- **Status:** `in-review` for S83 intake only; the package has no implementation authority.
...
- **Deferred:** S84 until S83 acceptance; all pilots, template changes and cross-runtime work until their preceding gate.
