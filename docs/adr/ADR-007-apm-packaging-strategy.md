...
- Hypothesis **H2** of the cross-repo plan ("APM accelerates adoption without excessive maintenance cost") is partially falsified: APM accelerates *versioning, distribution, and resolution*, but does not (today) accelerate *materialization* of project scaffolds. The remaining manual step is small but real, and must be tracked across the soft launch (Epic 8) to determine if it is a significant friction.
...
- **Option Z — pause and escalate.** Rejected because Epic 4 has downstream dependencies (Epics 5, 6, 8) and pausing would cascade. The structural mismatch is worth documenting (this ADR), not worth blocking on.
...
- **Upstream issue requesting APM lifecycle hooks.** Not rejected — recommended as a parallel action, but not blocking on Epic 4. The upstream timeline is unknown.
...
- **ADR-005** positions AletheIA as a consumer of APM in the ecosystem map. This ADR honors that position by using APM natively where possible and not building a custom CLI as escape valve.
...
- **2026-05-21 cross-repo plan, Epic 5** (Adaptive Skills as APM package) is the sibling effort. Adaptive Skills will face a different mismatch (its primitives map more cleanly to APM `skill` type) and likely use a different layout. This ADR does not constrain that decision.
...
- A soft-launch adopter (Epic 8) reports the two-step flow as significant friction, or fails to complete adoption because of it.
...
- The Adaptive Skills APM packaging (Epic 5) settles on a layout that suggests a different convention for AletheIA.
